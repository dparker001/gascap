...
function SignInForm() {
  ...
  // ── Biometric ────────────────────────────────────────────────────────────────
  // hasBiometricCredentials() is a safe Preferences read — no native UI, safe on mount.
  // getBiometricType() reads device capability only — no prompt.
  const [bioType,    setBioType]    = useState<BiometricType>(null);
  const [bioLoading, setBioLoading] = useState(false);

  useEffect(() => {
    (async () => {
      const [has, type] = await Promise.all([hasBiometricCredentials(), getBiometricType()]);
      if (has && type) setBioType(type);
    })();
  }, []);

  async function handleBiometricSignIn() {
    setBioLoading(true);
    const creds = await authenticateAndLoad();
    if (!creds) { setBioLoading(false); return; }
    const res = await signIn('credentials', { redirect: false, email: creds.email, password: creds.password });
    setBioLoading(false);
    if (res?.ok) redirect();
    else setBioType(null); // credentials stale — fall back to normal sign-in silently
  }
          ...
          {bioType && (
            <>
              <button
                ...
                disabled={bioLoading}
                className="w-full flex items-center justify-center gap-2.5 py-3.5 px-4
                           bg-slate-900 text-white rounded-2xl font-bold text-sm
                           active:opacity-80 transition-opacity disabled:opacity-60 mb-4"
              >
                {bioType === 'faceId' ? (
                ...
                {bioLoading
                  ? 'Authenticating…'
                  : bioType === 'faceId' ? 'Sign in with Face ID'
                  : bioType === 'touchId' ? 'Sign in with Touch ID'
