...
export async function POST(req: Request) {
  ...
  if (!user?.isTestAccount && existing.length >= limit) {
    const msg = plan === 'free'
      ? 'Free accounts can save 1 vehicle. Upgrade to Pro for unlimited vehicles.'
      : `Vehicle limit reached (${limit}). Upgrade to Pro at gascap.app/upgrade for unlimited vehicles.`;
