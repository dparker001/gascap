...
- Saved Vehicles: tap the ⭐ star on any saved vehicle to make it the default — it auto-applies in the calculator whenever nothing else is selected, most notably right after turning off Rental Car Return Mode, so the user's regular vehicle comes back automatically instead of needing manual reselection. Only one vehicle can be default at a time.
