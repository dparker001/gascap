...
- Fuel Calculator: calculates exact gallons needed to avoid overfilling (saves ~$0.40+ per fill-up vs industry-average pump overfill); set your current fuel level by dragging the needle on the fuel gauge dial or using the slider. Pro users can also tap "Scan Gauge" to photograph the dashboard gauge — GasCap™ locates the needle/E/F, computes the level from the needle angle, cross-checks it, and lets the user confirm or adjust via slider before applying
