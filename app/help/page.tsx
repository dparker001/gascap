  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: 'Rental Car Return Mode is designed for rental vehicle drop-offs. Tap the "🚗 Rental Car Return Mode" toggle at the top of the calculator to activate it — or set your Driver Mode to "Rental Car" in Settings and it activates automatically every time you open the app. Visiting gascap.app/rental and tapping the CTA also launches the calculator in rental mode directly. Your saved garage is hidden and replaced with rental car class presets (Economy, Midsize, SUV, etc.) for a quick tank-size estimate. If you know the exact car, you can also look it up by Year/Make/Model, or scan/enter its VIN (found on the dash or door jamb, same as any car) for the most accurate tank size. Enter the rental company\'s per-gallon rate to see exactly how much you save by fueling up yourself vs. returning empty. Renting an EV? Switch to the EV Charge tab with rental mode on — EV rentals require returning at a set charge level rather than a full tank, so GasCap\u2122 asks which company you rented from (Hertz, Avis, Budget, SIXT, Dollar, Thrifty, Enterprise) and your charge at pickup, then tells you the required return percentage, how many kWh you need, what it costs at your electricity rate, and how long it takes on a Level 2 charger.',
      },
      {
        q: 'What is the Rental Return Assistant?',
        a: 'The Rental Return Assistant (gascap.app/rental-return) is a saved, ongoing rental session — different from the quick Rental Car Return Mode toggle in the calculator, which resets each time. Set up a rental once (company, vehicle, pickup fuel level, required return level, the rental company\'s fuel rate if known, and your return location/time) and GasCap™ tracks it: it shows exactly how many gallons to add, an estimated cost, an estimated rental-company charge and potential savings if you know their rate, and lets you search gas stations near your return location specifically. Log an "I Just Refueled" entry any time to update your fuel estimate, and tap "Complete Rental" when you drop the car off — GasCap™ saves it to your Rental History along with whether you were charged a fuel fee, so you can see whether using GasCap™ actually helped. All fuel quantities from manual entry (gauge, percentage, or typed gallons) are shown as estimates, never exact measurements. You can also edit any active rental (vehicle, tank size, return requirement, rate, return location/time) or cancel it from the Edit button on the dashboard. To save typing, you can upload your rental agreement (the emailed PDF or a photo of it) at the first setup step and GasCap\u2122 will read it with AI and pre-fill what it finds \u2014 company, agreement and confirmation numbers, vehicle, return time and location, and the per-gallon refueling rate \u2014 all of which you can review and edit before saving (agreement scanning is a Pro feature; manual entry is always available). Both a rental agreement number and a confirmation number can be stored, since companies differ on which they issue \u2014 Hertz uses a confirmation number, Avis often gives both. If you enter a rental before you\'ve picked it up, leave the pickup fuel level blank — the rental shows as \u201cUpcoming,\u201d GasCap\u2122 reminds you 24 hours and again ~2 hours before pickup, and you set the actual fuel level from the dashboard when you collect the car (it can be corrected any time after, too). That number matters: under the default same-as-pickup policy it also becomes your return target. You can also optionally attach photos at both pickup (vehicle, gauge, rental agreement) and return (gauge, final receipt) for your own records — helpful if a fuel-fee dispute comes up later. The refuel log shows a running total (gallons added and spent so far) — useful on longer rentals with several fill-ups — and when you complete the rental, GasCap™ shows a recap of what you actually paid versus what the rental company would have charged for the same gallons, so you can see the real result. Tapping "Rental Car Mode" on the calculator always takes you straight to your active rental (or starts a new one); it never blocks you from using the regular calculator for your own garage vehicle at the same time.',
      },
      {
