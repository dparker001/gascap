...
export default function FillupLogger({ prefill, onSaved, onCancel, drivers = [] }: FillupLoggerProps) {
  ...
  // Set when the save was refused by the free-tier monthly cap, so the error
  // can carry an upgrade path instead of being a dead end.
  ...
  async function handleSave(force = false) {
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json() as { error?: string; proRequired?: boolean };
        setError(d.error ?? t.fillup.errSaveFailed);
        if (res.status === 403 && d.proRequired) setCapReached(true);
        return;
      }
      ...
      {error && (
        capReached ? (
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-3 space-y-2">
            <p className="text-xs text-amber-800 font-semibold leading-snug">{error}</p>
            <a href="/upgrade" className="inline-block px-4 py-2 bg-amber-500 hover:bg-amber-400 text-white text-xs font-black rounded-xl transition-colors">
