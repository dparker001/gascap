/**
 * Turns the 2026-07-25 raw captures into polished store screenshots:
 * brand-gradient background + headline caption + the framed app screen.
 * Outputs Apple 6.7", Apple 6.5", Play, and iPad sets.
 *
 * Raw captures live in public/store-screenshots/raw-2026/.
 * Run: node scripts/generate-store-screenshots-2026.mjs
 */
...
const dir = join(__dirname, '..', 'public', 'store-screenshots', 'raw-2026');
const outRoot = join(__dirname, '..', 'public', 'store-screenshots', 'final');

const GREEN = '#005F4A', TEAL = '#1EB68F';

const SHOTS = [
  { src: '01-calculator.png', name: '01-know-before-you-go', lines: ['Know your exact', 'fill-up cost'],   sub: 'No guessing. No overpaying.' },
...
for (const s of SHOTS) {
  await makeShot(s, 1290, 2796, join(outRoot, 'apple-2026'));
  await makeShot(s, 1284, 2778, join(outRoot, 'apple-6.5-2026'));
  await makeShot(s, 1080, 2160, join(outRoot, 'play-2026'));
  ...
  await makeShot(s, 2048, 2732, join(outRoot, 'ipad-2026'));
}
console.log('\n2026 store screenshots generated (Apple 6.7" + Apple 6.5" + Play + iPad).');
